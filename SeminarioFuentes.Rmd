...
En este estudio se analiza la correlación y causalidad de los suicidios, la situación económica y la inaccesibilidad a sectores de salud mental. En estos casos ninguna de esas variables explica la cantidad de suicidios que se dan en cada comunidad autónoma, sin embargo, la inaccesibilidad a servicios de salud mental sí está relacionada con la desigualdad económica de cada CCAA.
...
Para medir la desigualdad se pueden utilizar distintos indicadores, entre ellos encontramos el cociente S80/S20. Éste mide, según la INE (Instituto Nacional de Estadística),la desigualdad a través de ratios entre percentiles. Se interpreta como la relación entre la renta media obtenida por el 20% de la población con la renta más alta (quintil más alto), en relación a la renta media obtenida por el 20% de la población con la renta más baja (quintil más bajo) (1).
...
## Carga de los datos
### Carga de librerías
Se han utilizado las librerías de "readr" y "tidyverse" para poder alterar los set de datos tanto en csv como en json, las librerías "rjson" y "tidyjson" para poder manejar las de json únicamente. Además se ha añadido la librería cowplot para poder mostrar gráficas agrupadas.
```{r CargaLibrerías}
library(readr)
library(tidyverse)
library(rjson)
...
En este caso se ha hecho un join con el atributo "CA" entre suicidios y desigualdad con la tabla "sude" como resultado. Como se necesitan los datos de la población, se ha hecho join con este set de datos y con sude con el mismo atributo "CA". Mediante pipes se ha alterado la tabla "sude" seleccionando las columnas de interes y atribuyendo nuevos nombres a las columnas que por defecto tomaron nombres de Total.x y Total.y. También se ha alterado el nombre de s20s80 y se ha cambiado el orden de los atributos para que se vean primero las CCAA. Se han convertido los datos de interés a numérico y se ha hecho la proporción entre suicidios y el total de población para cada comunidad.
```{r tablasude}
...
Como en el caso anterior se ha hecho dos join entre los datos de suicidios, inaccesibilidad y población mediante el atributo "CA". Mediante pipes se ha alterado la tabla "suina" seleccionando las columnas de interes y atribuyendo nuevos nombres a las columnas y poniendo , como en el caso anterior la columna "CA" al principio. En este caso se ha filtrado solo aquellos datos en los que sí ha habido inaccesibilidad, del tipo de salud mental y para ambos sexos.
El atributo TotalInaccesibilidad se ha convertido a numérico, como habñia números decimales, se ha cambiado el separador "," por ".". Por último se ha hecho la proporción de suicidios e inaccesibilidad dividiendo cada dato por la población de su "CA" y multiplicando por 100.
```{r tablasuina}
...
En este caso se han seguido todo slos pasos de la tabla anterior, sin embargo, se ha filtrado por la inaccesibilidad a la atención médica en vez de mental.
```{r tablasuina2}
...
Como en el caso anterior se ha filtrado por otro tipo de inaccesibilidad (medicamento recetado) siguiendo los pasos de las tablas anteriores.
```{r tablasuina3}
...
Para este caso, se ha hecho un join entre el set de datos de desigualdad, inaccesibilidad y población mediante el atributo "CA". Se han renombrado algunos atributos, filtrado por la inaccesibilidad a sectores de salud mental en ambos sexos y se ha colocado la columna "CA" al principio. Además se ha pasado a numérico la columna de los valores que no lo estaban y como son decimales se ha sustituído el separador "," por ".". Como en casos anteriores se ha hecho la proporción entre los datos y  la población de cada comunidad.
```{r tabladesina}
...
## Gráficas
A partir de los datos anteriores se han diseñado gráficos de dispersión con regresiones lineales para poder estudiar las variables.
...
Haciendo una regresión lineal observamos que a simple vista hay un ligero aumento de suicidios a medida que 
```{r GraficaSuina1.2}
suina_plot2.1 <-
  ggplot(data = suina, aes(x = porPobIna, y = porPobSui)) +
  geom_point()+
...
```
Analizando la regresión lineal observamos que el ajuste no es significativo ya que el p-valor es mayor que 0.05 y la R muy alejada de 1. No hay causalidad entre las variables.
``` {r RL2}
#Regresión lineal
...
En este caso vemos que haciendo el ajuste no hay mucha variabilidad entre comunidades, aunque haya algunos puntos que estén alejados del ajuste lineal.
```{r GraficaSuina2.1}
...
suina2_plot2.1
```
Obtenemos una correlación negativa muy pequeña, por lo que no es significativo en nuestro estudio. No se puede asumir relación entre variables.
```{r CorrelacionSuina2.1}
#Correlación Suina
cor(suina2$porPobIna,suina2$porPobSui) #(x,y)
```

Como se puede observar en el resumen de la regresión lineal, el p-valor es muy alto (>0.05) y la R cuadrada ajustada muy alejada de 1, por lo tanto no hay causalidad entre variables.
``` {r RL3}
#Regresión lineal
...
```
En el caso de la regresión lineal vemos que el p-valor es muy elevado y la R cuadrada no es apta para un modelo bien ajustado, por lo que no existe causalidad entre variables.
```{r RL4}
#Regresión lineal
...
Haciendo una correlación podemos observar que es un valor negativo (-0.44) y cercano a -0.5 por lo que sí hay relación entre variables, existe una correlación negativa y débil entre ambas.
```{r CorrelacionDesina}
#Correlación Suina
cor(desina$s80s20,desina$porPobIna) #(x,y)
```
Analizando la regresión lineal vemos que el modelo no es significativo ya que la R cuadrado ajustada tiene un valor muy alejadod e 1 y el p-valor es ligeramente mayor que 0.05 (0.05671). No se puede deducir causalidad entre variables.
```{r RL5}
#Regresión lineal
...
# Conclusiones
* Hay cierta relación negativa entre la rentas80s20 y los suicidios en españa, de forma que cuanto mayor es la renta menor suicidios hay. Sin embargo, la cantidad de renta por CCAA no determina la cantidad de suicios que habrá.

* Hay una ligera correlación entre la cantidad de suicidios en las CCAA y la inaccesibilidad de cada una a servicios de salud mental de forma que cuanto mayor es la inaccesibilidad mayor cantidad de suicidios, sin embargo una no es la causa de la otra.

* Existe una correlación negativa entre la desigualdad y la inaccesibilidad a servicios de salud mental, de forma que cuanto mayor es cociente s80s20, menos inaccesibilidad presentan las CCAA en estos servicios. En definitiva cuanto mejor es situación ecónomica de los españoles mayor es el  acceso a determinados servicios de salud mental, sin embargo, no se ha demostrado causalidad entre ambos.

*De los tres tipos de inaccesibilidad (mental, por atención médica y medicamento recetado) tan sólo existe relación con los suicidios en el caso de la salud mental.

De esta forma conluimos que los suicidios son actos muy difíciles de estudiar ya que el motor que lleva a  cometer un suicidio puede ser cualquier motivo personal que se escapa a cualquier análisis, es por eso que no se han encontrado relaciones muy fuertes entre este fenómeno, la situación económica y la inaccesibilidad a servicios de salud mental. 
